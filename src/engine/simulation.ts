...
export function tick(state: GameState): GameState {
  ...
  if (run.year === WINTER_START) {
    ...
    const winterMsg = `The Great Cold begins. Farming disabled — food will be scarce. Food: ${Math.floor(resources.food)} (spoilage: ${winterSpoilage.toFixed(1)}/yr).${preservedNote}`;
